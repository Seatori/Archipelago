...
# TODO: Replace with NeoForge Installer (Lexi's Implementation)
version_file_endpoint = ""
neoforge_server_url = ""
...
class MinecraftClient(MDApp):
    ...
    def get_jdk(self) -> Optional[str]:
        jdk_exe = None
        for directory in os.listdir(options.server_directory):
            if directory.startswith(f"jdk{self.version['java']}") or directory.startswith(
                    f"amazon-corretto-{self.version['java']}"):
    ...
    # TODO: Get location from NeoForge Server Installer (Lexi's Implementation)
    def get_server_jar_name(self):
        return f"neoforge-server-mc.{self.version['minecraft']}-{self.version['neoforge']}-{self.version['neoforge_installer']}.jar"
